...
def combine_markdown_files(filenames, prefix):
    ...
    # Extract header only from the first file and adjust label to "Database"
    with open(filenames[0], 'r') as file:
        lines = file.readlines()
        # Adjust the first label in the header
        header_parts = lines[0].split("|")
        header_parts[1] = " Database "  # Adjust the first label
        adjusted_header = "|".join(header_parts)
        separator = lines[1]

        combined_rows.append(adjusted_header.strip() + "\n")  # Adjusted header
        combined_rows.append(separator.strip() + "\n")  # Separator

    # Extract body from each file
    for idx, filename in enumerate(filenames):
        with open(filename, 'r') as file:
            lines = file.readlines()
            start_index = 2  # Skip the header and separator for all files
            for line in lines[start_index:]:
                combined_rows.append(line.strip() + "\n")
